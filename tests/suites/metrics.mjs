...
/*
  ...
  has added, and it is on the landing page by choice.

  Then 30 -> 33 when that mark became an object — a depth per dot, a rotation
  matrix, a perspective divide and an insertion pass to keep the painting order
  honest. It is the second budget this one feature has moved, and that is worth
  naming rather than absorbing: about 1 kB of arithmetic, written out instead of
  imported, on a page whose own argument is that it does not import a hundred
  and fifty for this. Still around a fortieth of the JavaScript on a median
  page, which is the comparison the row underneath actually makes, and the row
  is what a visitor reads.
*/
ck('JS is a small slice of the page (< 33 kB)', num(js) > 0 && num(js) < 33, js);
