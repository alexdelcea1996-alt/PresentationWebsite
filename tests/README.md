...
1707 de verificări care rulează peste site-ul construit, majoritatea cu un browser real.
...
|---|---|---|
| `a11y` | axe-core (WCAG 2.1 AA) pe 30 de pagini, temă întunecată, plus greutatea paginii și inelul de focus de pe cardurile configuratorului — pe care axe nu-l poate vedea, fiindcă input-ul e decupat de `sr-only` împreună cu inelul lui | 68 |
| `a11y-light` | aceleași reguli pe 11 pagini în tema luminoasă | 38 |
| `csp` | zero violări CSP pe 6 pagini, cu header-ele reale aplicate | 16 |
| `interact` | meniu mobil, comutator de limbă, ancore, formularul în pași, ambele căi de trimitere — iar un formular neatins nu mai trimite nici buget, nici tip de proiect: selecturile pornesc pe o opțiune goală, etichetată, nu pe prima din listă, pe ruta Web3Forms ca și pe cea mailto — și starea de scroll fără să citească poziția de scroll (niciun listener de `scroll` pe pagină, santinelele la locul lor) | 57 |
...
| `hero` | glow-ul care urmărește cursorul (pe patru pagini, ambele teme) și parallaxul din hero; ambele oprite sub `prefers-reduced-motion` și pe pointer grosier; plus cele trei cifre de sub titlu: un singur rând la 320, 390 și 1280 px, nimic nu iese din celula lui, iar eticheta e în markup o singură dată, înaintea cifrei, și desenată sub ea | 44 |
| `booking` | modalul Cal.com, încărcare la cerere, temă, Escape, click cu modificatori — și același comportament în banda de final a paginilor care vând, a paginilor de serviciu și a articolelor: link real, nimic cerut calendarului înainte de click, un singur dialog pe pagină, calendarul în limba paginii | 66 |
...
| `pages` | site-ul nu mai e o singură pagină: fiecare dintre cele cinci categorii există în ambele limbi, e legată din header și se marchează ca pagină curentă, are titlu, descriere, hreflang și breadcrumb proprii — iar breadcrumb-ul vizibil spune, cuvânt cu cuvânt, același drum ca cel din JSON-LD, cu ultima treaptă marcată ca pagină curentă și nelegată; nicio secțiune nu apare de două ori la lungime întreagă (rezumatul de pe prima pagină e măsurat față de pagina lui); linkurile publicate înainte de despărțire ajung tot acolo, cu tot cu parametri; iar estimarea și auditul își duc contextul peste navigare și formularul îl uită după ce l-a folosit, selecturi incluse; plus că o frază care trimite „mai sus" sau „mai jos" la o secțiune o găsește pe aceeași pagină, în direcția spusă — două rămăseseră din vremea paginii unice și trimiteau la secțiuni mutate pe alte pagini | 149 |
| `completeness` | pagina 404, setul de iconuri, manifestul, schema `FAQPage`, `hreflang` în sitemap, `/version.txt`, HSTS, regulile de cache, `security.txt` (inclusiv expirarea recalculată la fiecare build), niciun hostname scris de mână în `src/` sau `tests/`, entitatea de afacere, `OfferCatalog`, breadcrumbs, FAQ-ul de pe prima pagină, secțiunea „Despre mine", fiecare link intern din fiecare articol, lista „Citește și" (fiecare pagină care vinde și fiecare articol se termină cu două-trei articole reale, în limba paginii, niciodată pagina însăși, niciun articol fără vreo listă care să ducă la el, prețurile trimit la ambele articole despre cost, iar pagina de contact nu are listă), și pagina pe care o vede un om când deschide adresa fluxului RSS | 173 |
| `guarantees` | secțiunea de garanții, ambele coloane, și că nu au rămas casete goale; plus că ce numește hero-ul „garantat" e scris în garanții, cu aceeași cifră, cu testul numit și cu telefonul inclus, iar coloana de alături spune unde se oprește promisiunea; că orice paragraf din site care leagă testul de viteză de predare poartă aceeași cifră — în garanții, în „Despre mine", pe paginile de serviciu și pe demo-ul de landing — la fel și cardul Landing page din lista de servicii; și că nicio pagină nu promite un timp de încărcare în secunde | 41 |
...
| `offers` | că aceeași ofertă are același preț în carduri, pe pagina de serviciu, în configurator, în formular și în articolele de blog; același termen pe card, pe pagina de serviciu, în hero (care spune pentru ce pachet e adevărat) și în estimarea fără extra; prețul de pornire de pe prima pagină și din descrierea paginii de prețuri e citit din carduri, iar nimic nu mai numără pachetele în cuvinte; nicio pagină nu pretinde o popularitate pe care n-o poate arăta; plus că linkurile din articole chiar răspund | 134 |
...
| `colophon` | colofonul și lista de lansare: cifra de pe pagină e cea din depozit, bugetele citate sunt cele asertate, fiecare suită citată există, pagina nu-și declară niciodată propriile teste trecute, iar lista se tipărește; plus că blocul cu cifra și desenul iese din coloana de text pe ecran lat, desenul are 420 px, cifra e mare, iar pe telefon (320 și 390 px) nimic din card nu iese din el | 73 |
