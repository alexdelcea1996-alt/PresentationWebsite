...
const csp = [
  ...
  // 'self' because the demo pages frame the example sites, which are pages of
  // this site served from this origin.
  `frame-src 'self'${bookingOrigins}`,
  "base-uri 'none'",
  "object-src 'none'",
  // Relaxed from 'none' for the same reason. Third-party framing — the actual
  // clickjacking risk — is still refused; only this origin may frame itself.
...
# The feed's own viewer. Cloudflare has no content type for .xsl, and with
# X-Content-Type-Options: nosniff set above, a stylesheet served as a byte
# stream is one the browser refuses to apply — the feed would print as source
